...
export default function AttendancePage() {
    ...
    const formatTime = (isoString?: string) => {
        if (!isoString) return '-';
        return new Date(isoString).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false }); // 24h format as per pic 10:00 - 19:00
    };

    // Filter by Search (Client-side for now as per requirements/simplicity)
    const filteredRecords = records.filter(rec =>
        `${rec.employeeId.firstName} ${rec.employeeId.lastName}`.toLowerCase().includes(search.toLowerCase())
    );
        ...
        <div className={styles.container}>
            ...
            <div className={styles.controls}>
                ...
                <div className={styles.dateDisplay}>
                    {isAdmin ? (
                        <div className={styles.datePickerWrapper} onClick={() => adminDateRef.current?.showPicker()}>
                            <input
                                ref={adminDateRef}
                                type="date"
                        ...
                        <div className={styles.datePickerWrapper} onClick={() => empDateRef.current?.showPicker()}>
                            <input
                                ref={empDateRef}
                                type="month"
                                value={`${currentMonth.getFullYear()}-${(currentMonth.getMonth() + 1).toString().padStart(2, '0')}`}
                                onChange={(e) => {
                                    if (e.target.valueAsDate) setCurrentMonth(e.target.valueAsDate);
                                }}
                                className={styles.hiddenDateInput}
                            />
                            <div className={styles.dateLabelClickable}>
            ...
            <div className={styles.tableCard}>
                <div className={styles.tableHeaderDate}>
                    {isAdmin ? currentDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) : currentMonth.toLocaleDateString('en-GB', { month: 'long', year: 'numeric' })}
                </div>
                    ...
                    <tbody>
                        {filteredRecords.length > 0 ? (
                            filteredRecords.map((rec) => (
                                <tr key={rec._id}>
                                    <td>{rec.employeeId.firstName} {rec.employeeId.lastName}</td>
                                    <td>{formatTime(rec.checkIn)}</td>
                                    <td>{formatTime(rec.checkOut)}</td>
                                    <td>{rec.workHours ? formatTimeFromHours(rec.workHours) : '-'}</td>
                                    <td>{rec.extraHours ? formatTimeFromHours(rec.extraHours) : '-'}</td>
                                </tr>
                            ))
                        ) : (
